# Translator Prompt — query → English + language tag

Converts the user's query into English for downstream retrieval and reports which
language/script it was written in. Never answers the medical question, never adds
dosage advice or warnings, never adds information the user didn't write.
